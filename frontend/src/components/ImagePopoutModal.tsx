...
export default function ImagePopoutModal({ visible, imageUri, post, onClose, onOpenComments, onReactRequest, onLikeToggled }: ImagePopoutModalProps) {
  ...
  const recentReactions = post?.recentReactions || [];
                   ...
                   <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }} pointerEvents="box-none">
                     {recentReactions.length > 0 && (
                        <View style={{ flexDirection: 'row-reverse' }}>
                          {recentReactions.slice(0,3).map((uri: string, idx: number) => (
